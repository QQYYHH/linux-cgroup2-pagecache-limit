 ...
 * 如果当前CPU缓存的是该 memcg对应的页面，且 缓存的页面数量多余需求
 * 成功
 */
 ...
 * 填充当前CPU 的 stock缓存
 * 如果当前CPU缓存的不是 memcg
 * 调用drain_stock 将多余的page从对应的page_counter减去
 * 然后重置 当前CPU的缓存
 */
 ...
 * 在@ root_memcg 所在的层级，清空所有 cpu 缓存的 stock
 * 旨在 释放更多空闲页
 */
...
done_restock:
	 ...
	 * refill_stock
	 * 填充 当前CPU的 stock缓存
	 * 因为 batch对应的页面数量 多余需求nr_pages
	 * 但是 page_counter_charge一次会 加batch个页面
	 */
	if (batch > nr_pages)
