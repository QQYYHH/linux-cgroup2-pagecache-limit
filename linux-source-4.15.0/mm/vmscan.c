...
unsigned long lruvec_lru_size(struct lruvec *lruvec, enum lru_list lru, int zone_idx)
{
	...
	/**
	 * 减去 其他 zone区的 链表数量
	 * 只求 zone[zone_index] 区的链表
	 * 如果zone_index设置成 MAX_NR_ZONES，则不与特定的zone区相关
	 */
			 ...
			 struct scan_control *sc)
{
	 ...
	 * 判断是否可以释放该page
	 * 该page的引用次数必须 == 3
	 * 当前回收进程PageReclaim + radix_tree + buffer_heads at page->private
	 * 如果不行，直接返回 PAGE_KEEP 表示当前page还不能写回
	 */
				      ...
				      bool force_reclaim)
{
	...
	/* 页面被锁住，继续放入 inactive_list中 */
		...
		/* sc结构体标记了不允许unmap操作且该页面对应有pte，继续保留在inactive_list */
		...
		/**
		 * 调用 __remove_mapping 将页面归还 伙伴系统
		 */
		if (PageAnon(page) && !PageSwapBacked(page)) {
...
keep_locked:
		unlock_page(page);
keep:/* 将页面继续留在 inactive_list中，下次执行shrink_page_list的时候再处理 */
...
/**
 * @brief 将page_list中没有被回收的页面放回到lruvec 合适的LRU链表中
 * 但是并非page_list中每个page都不可回收
 * 再次检查每个页面是否可回收，如果可以，立刻回收并重新加入到page_list
 * 
 * @param lruvec 
 * @param page_list 
 */
...
putback_inactive_pages(struct lruvec *lruvec, struct list_head *page_list)
{
	...
	while (!list_empty(page_list)) {
		...
		/* 测试该page是否可用于回收 */
		/* 如果测试通过，该page会被填充为全 0 */
		/* 相当于被回收，需要统计page_counter资源 */
		if (put_page_testzero(page)) {
			__ClearPageLRU(page);
		     ...
		     struct scan_control *sc, enum lru_list lru)
{
	...
	*/
	...
	/**
	 * 核心的释放处理函数
	 * 执行完毕之后，page_list中是 剩下没有被释放的page
	 */
	 ...
	 * 并再次检查每个页面是否可回收，如果可以，立刻回收并重新加入page_list
	 */
...
/* 将某些页面转移： active_list -> inactive_list */
				 ...
				 struct scan_control *sc, bool trace)
{
	...
	/* active : inactive = inactive_ratio : 1 */
	/* inactive * inactive_ratio = active */
	/* 如果 < 说明 inactive_size 偏低 */
 ...
 * @pgdat: struct pglist_data *pgdat 指定了每个 node 的 内存布局
 */
...
retry:
	 ...
	 * 如果没有满足 回收页面数量要求，会首先递减优先级（优先级越低，每次扫描的页面数量越多）
	 * 然后开始新一轮的释放，直到满足要求
	 */
	do {
